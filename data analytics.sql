...
SELECT
	DATE_FORMAT(v.view_at, '%Y-%m') AS month,
	COUNT(DISTINCT v.user_id, v.user_session) AS view,
    COUNT(DISTINCT c.user_id, c.user_session) AS cart,
	ROUND(COUNT(DISTINCT c.user_id, c.user_session) * 100.0 / COUNT(DISTINCT v.user_id, v.user_session), 2) AS view_cart_conversion
FROM views AS v
LEFT JOIN carts AS c
ON v.user_id = c.user_id
    AND v.user_session = c.user_session
    AND v.view_at <= c.cart_at
GROUP BY month
ORDER BY month;
